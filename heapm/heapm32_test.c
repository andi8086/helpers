...
int main(void)
{
        ...
        TEST_CHECK(all_free == ctx.mem_size - sizeof(hm_root_info_t));

        void *p1 = hm_alloc(&ctx, 131072);

        all_free = hm_available(&ctx, false);
        TEST_CHECK(all_free == ctx.mem_size - 131072 - sizeof(hm_pfx_t) -
                                   sizeof(hm_root_info_t));
        ...
        TEST_CHECK(hm_allocated(&ctx) ==
                   sizeof(hm_root_info_t) + 3 * sizeof(hm_pfx_t) + 3 * 65536);
        ...
        TEST_CHECK(hm_allocated(&ctx) ==
                   sizeof(hm_root_info_t) + 5 * sizeof(hm_pfx_t) + 5 * 65536);
        ...
        TEST_CHECK(hm_allocated(&ctx) == sizeof(hm_root_info_t) +
                                             4 * sizeof(hm_pfx_t) + 3 * 65536 +
                                             131072);
        ...
        for (int i = 0; i < 1080; i++) {
                ...
                calc = total_req + n_allocs * sizeof(hm_pfx_t) +
                       sizeof(hm_root_info_t);
                tree = hm_allocated(&ctx);
                TEST_CHECK(calc == tree);

                calc = ctx.mem_size - total_req - n_allocs * sizeof(hm_pfx_t) -
                       sizeof(hm_root_info_t);
        ...
        TEST_CHECK(((hm_pfx_t *)p[0] - 1)->malloc_line == 185);
